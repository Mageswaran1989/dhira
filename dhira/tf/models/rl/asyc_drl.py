...
from dhira.data.openai.atari_environment import AtariEnvironment
import threading
...
def build_network(num_actions, agent_history_length, resized_width, resized_height):
  with tf.device("/cpu:0"):
    ...
    model = Conv2D(activation="relu", kernel_size=(8, 8), filters=16, strides=(4, 4), padding="same")(inputs)
    model = Conv2D(activation="relu", kernel_size=(4, 4), filters=32, strides=(2, 2), padding="same")(model)
    model = Flatten()(model)
    ...
    while T < TMAX:
        ...
        while True:
            ...
            if terminal:
                ...
                print
                "THREAD:", thread_id, "/ TIME", T, "/ TIMESTEP", t, "/ EPSILON", epsilon, "/ REWARD", ep_reward, "/ Q_MAX %.4f" % (
